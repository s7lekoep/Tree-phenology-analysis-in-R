...
* The phonological stages are standardized (scale from 0 to 9). That makes it esay to recognize them under all field conditions across species. This allows to creat an easily comparable metric for classification. 

* The combination of the principal and the secondary status (also scale from 0 to 9) results in the 2 digital code. In this code the first digit represent the macro stage (principal growth stage) and the second digit a specific micro stage (secondary growth stages) to precise steps within.
...
# Lecture 04 Climate change and impact projection
...
* Sun:She warms the earth through solar radiation and her activity chnages slightly over decades, but not recent climate change. 
...
* Clouds: Have both warming effect(eg. warming night) and cooling (eg. sunny day) net effect. They are likely important but poorly understood. The effect depends probably on height, shape and latitude.
  
* Ozone: Stratospheric ozone: It`s Shields the earth from UV-B radiation through greenhouse gas. They have a         warming effect 
   destoyed by CFCs (Chlorofluorocarbon).
* Surface albedo: Surfaces reflect/absorb radiation dependent on their material (lbright surfaces e.g. ice reflect almost all radiation). -> landscape change lead to change of reflection (water adsorbent almost all reflaction).

* Greenhouse gases: Atmospheric gases that absorb long wavelength radiation (mostly) coming from earth (eg. Co2, Ch4, N2O). This are by far the biggest driver of the recent climate change.
...
* The last two millenniums up to a few decades ago the global climate was quite cold on average. However looking at the last decade the the average temperature keeps rising. This temperture increase brings the risk of a positive feedback, which consequences e.g. melting permafrost ande reduce the albedo.
If this increase would go on it would probably leads in unstable word. We don`t know what will happend but we know that humans have never experienced warming as rapid as we have seem in the recemt decades. 

3. What does the abbreviation ‘RCP’ stand for, how are RCPs defined, and what is their role in projecting future climates?

  * RCPs= Representative Concentration Pathways. This RCPs are the new generation of greenhouse forcing scenarios, which are the basement fore global climate models (GCMs). This golabal models are very computationally intensive. Therefor the they get downsampeld fpr specific topics. There are tow ways of downsampling: 
  + Dynamic downsampling: RCM (Regional climate model) are used with boundary conditions from GCMs
  advantage: delivers spatially coherent senarios.
  + Statistical downsampling: Weather generators or elta changes simply add differential based on present differences.  
...
* Climate analogue modles: Make use of the idea that for most projected climates for a given location, we can find a place where similar climate exists today.

# Lecture 05 Winter chill projections
...
* First, collect proper data on projected future climates.  * Second, choose a model and have it create scenarios.
...
# Lecture 06 How to write a function
...
A basic function that calculates warm hours (>25°C) with a random dataframe (`df`) could look like that:
```{r eval=FALSE }
...
The `Winters_hours_gaps` dataset was recorded in a walnut orchard near Winters, California, in 2008. This dataset has hourly time steps, which is necessary to compute the Chilling Hours. For the calculation we just need the date, time of the measurement and the tempertaur data. The other parameters of the data set can be ignored.
...
To calculate the temperature only within a certain period of time, the function must be extended.
...
# Lecture 08 Making hourly temperatures
...
The daylenght is an important parameter for the calculation of the daily temperature curve.  
Due to the fact that longer days have the potential to be significantly hotter than cooler ones. 
There are different ways to calculate the daily temperature curve one way is the idealized daily temperature curves. We can calculate this by using the example of the dataset `KA_waether`and the  `stack_hourly_temps` function of the `chillR` library. This function is prediction the ideal curve taking into account the latitude.
...
Therefore there exist fore example the empirical daily temperature curve. 
...
# Lecture 10 Filling gaps in temperature records
...
kable(fixed_data$QC, caption="Quality control summary produced by *fix_weather()*, with all days interpolated")
```
...
```{r echo=FALSE, results='asis'}
kable(station_list, caption="List of GSOD weather stations close to the weather station of Kallbadagrund")
```
3. Identify suitable weather stations for patching gaps. And download weather data for promising stations, convert them to `chillR` format and compile them in a list.

The station PORVOO KALLBADAGRUND looks most useful fore us to fill our gaps. 
```{r lad, eval=FALSE}
...
We can use the `fix_weather()` function to fill the gaps and with the qualitycontrol we can take a look at where the remaining gaps are:
```{r eval=FALSE }
...
```
We s´can see that there is no day missing. 
Now we can safe our dataset:
```{r eval=FALSE}
...
# Lecture 11 Generating temperature scenarios
...
`chillR` has a fuction called `temperature_generation`, which we can to produce synthetic temperature data. For example based on our data, we downloaded in the last lecture, for 100 years:
...
```
we can follow the course of the chill accumulation and see that it has a slight S-shape through the ends and the rest appearsto be relatively linear. 
...
# Chapter 12 Saving and loading data (and hiding this in markdown)
...
# Lecture 13 Historic temperature scenarios
...
In the last lecture we used a weather generator to produce agroclimatic profiles for Helsinki. 
The weather generator can also produce scenarios that represent different climatic conditions.
We can for example us it to generate a scenarios that represent particular years from the historic record. This has the advantage that we get to know what temperature conditions should have been considered normal at the time, rather than what actually happened. With this scenarios we can try to evaluate the gradual shift in climate.
But first we have to generate them: 
...
# Lecture 14 Future temperature scenarios
...
# Lecture 15 Plotting future scenarios
...
# Lecture 16 Chill model comparison
...
This calculation offers an opportunity for reducing the complexity of our simulation results by presenting our data in different formats lika a heat map. We will use the heat map to illustrate differences in the behavior of the various chill models that have been used in the literature (and sometimes in practice).
...
```
2. Make a heat map illustrating past and future changes in Safe Winter Chill(SWC), relative to a past scenario, for the 13 chill models used here.
````{r eval=FALSE}
...
knitr::include_graphics('AnimationGif.gif')

```


# Lecture 17 simple phenology analysis
...
As already mentioned we need to know the chilling period and the heat. In addition, however, we have to decide when the endo- and ecodormancy starts/ends.

# Lecture 18 Delineating temperature response phases with PLS regression
...
The analysis resulted into two outputs the model coefficients (middel graphic), which indicates the strength of the influence and the variable importance in the projection the vip (upper graphic), which point out the significant importance of the model coefficient value of 0.8 are considerd significant. The tilling period is considerd when the model coefficients present positive significant values more temperatuere delays blooming dates (here in green). Negativ significance values indicates that less temperature delays blooming dates. The lowest graph shows the mean temperature.
We can see (as expected) that warm temperature advancing the blooming date and to early heat (before january) can delay. But the last effect looks rather small, although the coefficient has a period longer positive period before. The reason for this could be because in the region around Campus Klein-Altendorf we always get enough chill at this time so the temperatures doesn`t matter as long they are quite similar to the normal temperatures in this area. 


# Lecture 19 Successes and limitations of PLS regression analysis
...
We could overcome this problem if, instead of temperature, we were calculate chill acumulation/heat and compare this with blooming date. With this approach we would have two parameters that have a more direct influence on the blooming date.
\[
\frac{Temp_{chilling}\rightarrow chill\rightarrow}{Temp_{forcing}\rightarrow heat\rightarrow } BloomDate
\]
...
# Lecture 20 PLS regression with agroclimatic metrics
...
```
# Lecture 21 Examples of PLS regression with agroclimatic metrics
...
Können Sie ein Muster erkennen, bei dem die Abkühlungs- und Treibphasen klar abgegrenzt werden konnten und bei dem dieser Versuch fehlschlug?

heat respose (forcing period) starts eg in januery

chilling pattern hart to see
rist model not sutebil for this tree (make for peach tree )
peach purnus see chilling and forcing -> model gemacht für diese daten  mittelpart nicht sicher weil warscheinlcih in phase die eh kalt genug 

beginning of cool sean -> clear respos of model coefficion and clrean respons at the end-> forcing 

long anclear period 
calefonier: warmer it is -> temp response clear

tunesion see  clrea chill respons aber kein forcing
...
really could not chilling 
than optimun und then if warmer no chilling pLS cant  work tryed to make it up in chill matrics heatdont have this problem becaus is never gets so hot that heat have no effect
...
Furthermore, the basic assumption that all temerature responses are the same may be wrong. Accordingly, one could not use the same model over the entire period. In a tree there are many different processes going on during this time and probably one would have to consider these more in the model.

# Lecture 22 Why PLS doesn’t always work
...
hkizfgjhkbjjjjjjjjjjjjjj

# Lecture 23 Evaluating PLS outputs
...
We see here an interpolated surface from all bloom dates included in the phenology record, which relates bloom date expectations to temperatures during the chilling phase (x-axis) and the forcing phase (y-axis). The shape of the plotted surface is defined by the array of winter conditions that have been observed since 1958. The white areas means, that there have been no years with this average temperature during chilling and forcing e.g. here -2°C at chilling and 4°C during forcing. 
...
# Lecture 24 The relative importance of chill and heat

1. Describe the temperature response hypothesis outcined in this chapter.
...
We can see that if its gets warmer the temperature response will follow the upward trend to earlier spring phases/earlier bloom dates. In cold reagions (blue part of the figure) is probably the chill accumulation so high, that the usual temperature variation during this period has little to no effect.
With increasing temperature the conditions during chill accumulation start to showing an effect (yellow part of the figure). Eventually the further increases in temperature is positively correlating to the effect of temperature conditions during chill accumulation, which take over as the dominant effect (red part of the figure). 
...
We make models for reasons, but rarely the reason is to model exact what we see. Most of the time we want to model how systems would respond to things we din`t try yet. 
...
# Lecture 27 The `PhenoFlex` model
...
state of the art in tree phenology we have to parameterize 12 parameter. 12 Parameters 
seems quite a lot but to get a critical mass of phenology data is sometimse quite hard, because it is a lot of efford to collect all needed data.
...
```
The problem is some parameter are hard to guess. The max and min values are easy, they are similar to optimum temperature. Harder to estimate are e.g. the structure of chill portion precursor, because we don´t really know how they respond to in terms of biology. We just know what the dynamic model included but we have to estimaed the upper and lower boundary. In addition the chill (yC) and heat (ZC) requirements are hard to guess because they can assume all kinds of values depending on how the other parameters are chosen. That`s why we run the solver to find optimal parameter values:
...
# Lecture 28 The PhenoFlex model - a second look
...
```
# Lecture 29 Can we improve the performance of PhenoFlex?

1. What was the objective of this work?

The goal was to assess the impact of marginal seasons on the predictive performance of PhenoFlex. The difficulty here lay in the selection of the right date set. Some times we getting a not fitting response curve for chilling. A reason for this could be that the data we use to train the model are unsuitable for finding out anything about the response on the winter season (Novermber-Januar), because there are less data points in this period. For the callibartion of the modelwe have to check how the model is responding depending on training data. That`s why we need a fitting procedure, where we store the randomized training data to get a better feeling for which results led to which output. 
  
2. What was the main conclusion?

The main conlusion that there is a limit of maginal data we should take into account for testing. It is reasenalble to Orinat the selection of point according to what we want to predict. Getting an impression on what climat conditions are predicted here in the future should gite our selection on our training an valisation data. In addition we have to be careful to add extrem tredments in such a analysis, because there could be some biological mechanisms that are not considerd in the `PhenoFLex` framework.
...
# Lecture 30 Frost risk analysis
...
knitr::include_graphics('Figures/Lectures30DD.png')
```
...
knitr::include_graphics('Figures/Lecture30d.png')
```
...
As can be seen in the analysis, frost risk for first, full and last blooming decreases over time. This can be seen in the significant p-values and the linear trents, which are all negative. 